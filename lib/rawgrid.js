...
const RawGrid = (() => {
  ...
  function render(data, cellPx) {
    ...
    // 7x7 finder: 3x3 black center, 1-cell white ring, 1-cell black outer ring
    // Every row has the 1:1:3:1:1 B:W:BBB:W:B ratio (no center white dot)
    ...
    for (var fi = 0; fi < finderPos.length; fi++) {
      ...
      for (var i = 0; i < 49; i++) {
        if (fp[i]) ctx.fillRect((off+fx+(i%7))*cellPx, (off+fy+Math.floor(i/7))*cellPx, cellPx, cellPx);
      }
    }
  ...
  function locateGrid(pixels, width, height) {
    ...
    // Verify finder: cross-section in both directions, allow 2 mismatches
    function isRealFinder(fc) {
      ...
      return mismatches <= 2;  // allow up to 2 errors out of 14
    }

    // Filter: only keep candidates that pass vertical cross-check
    finderCandidates = finderCandidates.filter(isRealFinder);
    if (finderCandidates.length < 3) return null;

    // Score by how well the ratio matches (tighter = better)
  ...
  function decode(pixels, width, height) {
    ...
    for (var gi = 0; gi < Math.min(grids.length, 30); gi++) {
      ...
      // Try gridSize ±4 and origin ±1 cell to correct estimation errors
      for (var dg = -4; dg <= 4; dg++) {
        for (var dx = -1; dx <= 1; dx++) {
          for (var dy = -1; dy <= 1; dy++) {
            var tryG = { x: g.x + dx * g.cellPx, y: g.y + dy * g.cellPx, cellPx: g.cellPx, gridSize: g.gridSize + dg };
            ...
            if (r && r.length >= 20) {
              ...
              var gsz = dv.getUint16(10, false);
              if (m === 0x4F42 && fk > 0 && fk <= 20 && gsz > 0 && gsz <= 30 && pl > 0 && 20 + pl <= r.length) {
                return r;
              }
            }
          }
