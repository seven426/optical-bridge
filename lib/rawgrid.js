...
const RawGrid = (() => {
  ...
  // Multi-point sampling: 5 points per cell (center + 4 corners), majority vote
  function sampleCells(pixels, width, imgH, grid, threshold) {
    ...
    var bi = 0;
    var pts = [[0,0], [-0.3,-0.3], [0.3,-0.3], [-0.3,0.3], [0.3,0.3]];

    for (var row = 0; row < GRID_SIZE && bi < DATA_BITS; row++) {
      for (var col = 0; col < GRID_SIZE && bi < DATA_BITS; col++) {
        if (isReserved(row, col)) continue;

        var cx = ox + (col + 0.5) * msX;
        var cy = oy + (row + 0.5) * msY;
        var votes = 0;

        for (var p = 0; p < 5; p++) {
          var px = Math.round(cx + pts[p][0] * msX);
          var py = Math.round(cy + pts[p][1] * msY);
          if (px >= 0 && px < width && py >= 0 && py < imgH) {
            if (getPixel(pixels, width, imgH, px, py) < threshold) votes++;
          }
        }

        if (votes >= 3) result[bi >> 3] |= (1 << (7 - (bi & 7)));
        bi++;
      }
    }
    return result;
  ...
  function validateHeader(bytes) {
    ...
    return m === 0x4F42 && fk > 0 && fk <= 20 && pl > 0 && 20 + pl <= bytes.length;
  }
  ...
  function decode(pixels, width, height) {
    ...
    // Step 5: Validate
    if (validateHeader(result)) return result;

    // Step 6: One retry with +0.5ms sub-pixel offset
    grid.originX += 0.5 * msX;
    grid.originY += 0.5 * msY;
    var result2 = sampleCells(pixels, width, imgH, grid, thresh);
    if (validateHeader(result2)) return result2;

    _lastDebug.error = 'header validation failed';
    return null;
  }
