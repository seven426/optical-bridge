...
const RawGrid = (() => {
  ...
  function locateGrid(pixels, width, height) {
    ...
    _lastDebug.rawCoords = finderCandidates.map(function(fc) {
      return '(' + Math.round((fc.top.sx + fc.bottom.ex) / 2) + ',' + Math.round((fc.top.y + fc.bottom.y) / 2) + ' ms=' + fc.moduleSize.toFixed(1) + ')';
    });
    ...
    _lastDebug.crossedCoords = finderCandidates.map(function(fc) {
      return '(' + fc.cx.toFixed(1) + ',' + fc.cy.toFixed(1) + ' ms=' + fc.moduleSize.toFixed(1) + ')';
    });
    if (finderCandidates.length < 2) {
