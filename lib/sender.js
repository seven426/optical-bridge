const Sender = (() => {
  ...
  async function prepareFile(file, opts = {}) {
    ...
    if (mode === 'camera') {
      ...
      payloadCapacity = C4Grid.dataCells();  // 7278
      maxPayloadPerFrame = FrameProtocol.innerDataCapacity(payloadCapacity, 212) - headerSize;
    } else {
  ...
  function generateFrames() {
    ...
    var isGrid = state.gridMode === 'colorgrid' || state.gridMode === 'c4grid';
    var gridCapacity = state.gridMode === 'c4grid' ? C4Grid.dataCells() : (state.gridMode === 'colorgrid' ? ColorGrid.dataCells() : 0);
    ...
    for (const gid of gids) {
      ...
      for (let i = 0; i < encoded.length; i++) {
        ...
        if (gridCapacity > 0) {
          var innerK2 = state.gridMode === 'c4grid' ? 212 : undefined;
          var innerEncoded = FrameProtocol.encodeInnerFEC(frame, gridCapacity, innerK2);
  ...
  function getCycleFrames() {
    ...
    var gridCapacity = state.gridMode === 'c4grid' ? C4Grid.dataCells() : (state.gridMode === 'colorgrid' ? ColorGrid.dataCells() : 0);
    var decodeInnerK = state.gridMode === 'c4grid' ? 212 : undefined;

    function unpackGridFrame(rawFrame) {
      if (gridCapacity > 0) {
        var decoded = FrameProtocol.decodeInnerFEC(rawFrame, gridCapacity, decodeInnerK);
