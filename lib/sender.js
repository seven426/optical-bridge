const Sender = (() => {
  ...
  function generateFrames() {
    ...
    // Pad last group to K frames, and pad Frame #0 to match group length if needed
    const gids = Object.keys(groups).map(Number).sort((a, b) => a - b);
    for (const gid of gids) {
      const groupData = groups[gid];
      // Pad Frame #0 (or any non-uniform frame) to match other frames in group
      var targetLen = maxPayloadPerFrame;
      if (gid === 0) targetLen = maxPayloadPerFrame; // Frame #0 padded to data frame length
      for (var i = 0; i < groupData.length; i++) {
        if (groupData[i].length < targetLen) {
          var p = new Uint8Array(targetLen);
          p.set(groupData[i]);
          groupData[i] = p;
        }
      }
      ...
      while (groupData.length < fecK) {
        groupData.push(new Uint8Array(targetLen));
      }
    }
  ...
  function getCycleFrames() {
    ...
    // 3 copies of frame #0 at the start for quick metadata acquisition
