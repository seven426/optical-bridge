const Sender = (() => {
  ...
  async function prepareFile(file, opts = {}) {
    ...
    if (mode === 'rawgrid') {
      // RawGrid: auto-size grid to hold at least this many bytes
      // Use a target similar to V15 for reasonable grid size
      var cellPx = opts.cellPx || 4;
