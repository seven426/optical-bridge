const Sender = (() => {
  ...
  async function prepareFile(file, opts = {}) {
    ...
    if (mode === 'rawgrid') {
      payloadCapacity = RawGrid.dataCells();  // 1213
    ...
    state = {
      ...
      qrVersion,
      ecLevel,
      gridMode: mode,  // render mode: rawgrid/colorgrid/camera/screen
  ...
  function generateFrames() {
    ...
    var gridCapacity = 0;
    if (state.gridMode === 'rawgrid') gridCapacity = RawGrid.dataCells();
    else if (state.gridMode === 'colorgrid') gridCapacity = ColorGrid.dataCells();
    ...
    for (const gid of gids) {
      ...
      for (let i = 0; i < encoded.length; i++) {
        ...
        // Apply inner FEC for grid modes
        if (gridCapacity > 0) {
