const Sender = (() => {
  ...
  function generateFrames() {
    ...
    // Equalize frame lengths within each group and pad last group to K frames
    const gids = Object.keys(groups).map(Number).sort((a, b) => a - b);
    for (const gid of gids) {
      const groupData = groups[gid];
      // Find max frame length in this group
      var maxLen = 0;
      for (var i = 0; i < groupData.length; i++) {
        if (groupData[i].length > maxLen) maxLen = groupData[i].length;
      }
      // Pad all frames in group to maxLen
      for (var i = 0; i < groupData.length; i++) {
        if (groupData[i].length < maxLen) {
          var p = new Uint8Array(maxLen);
          p.set(groupData[i]);
          groupData[i] = p;
        }
      }
      // Pad group to K frames (zero-filled)
      while (groupData.length < fecK) {
        groupData.push(new Uint8Array(maxLen));
      }
    }
  ...
  function getCycleFrames() {
    ...
    // All mode: distribute 3 copies of frame #0 at 0, 1/3, 2/3 positions
    var all = state.frames;
    var frame0 = all[0];
    var oneThird = Math.floor(all.length / 3);
    var twoThirds = Math.floor(2 * all.length / 3);

    var cycle = [frame0];
    for (var i = 1; i < all.length; i++) {
      if (i === oneThird || i === twoThirds) {
        cycle.push(frame0);
      }
      cycle.push(all[i]);
    }
