  ...
  function decode(pixels, width, height) {
    ...
    for (var row = 0; row < GRID_SIZE && bi < DATA_CELLS; row++) {
      for (var col = 0; col < GRID_SIZE && bi < DATA_CELLS; col++) {
        ...
        var px = Math.round(cx), py = Math.round(cy);
        var pix = getPixel(pixels, width, imgH, px, py);

        // R channel → bit in first 1213 bytes
        var rBit = pix.r < threshR ? 1 : 0;
        if (rBit) result[bi >> 3] |= (1 << (7 - (bi & 7)));
        // G channel → bit in second 1213 bytes
        var gBit = pix.g < threshG ? 1 : 0;
        if (gBit) result[(DATA_BYTES / 3 + (bi >> 3)) | 0] |= (1 << (7 - (bi & 7)));
        // B channel → bit in third 1213 bytes
        var bBit = pix.b < threshB ? 1 : 0;
        if (bBit) result[(2 * DATA_BYTES / 3 + (bi >> 3)) | 0] |= (1 << (7 - (bi & 7)));
