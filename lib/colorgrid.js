...
const ColorGrid = (() => {
  ...
  function locateFinders(pixels, width, imgH) {
    ...
    // Step 1: Horizontal scan — inline luma to avoid per-pixel object allocation
    var rowHits = [];
  ...
  function decode(pixels, width, height) {
    ...
    var tlcx = Math.round(pair.tl.cx), tlcy = Math.round(pair.tl.cy);
    var tlbx = Math.round(pair.tl.cx - 2 * msX), tlby = Math.round(pair.tl.cy);
    var brcx = Math.round(pair.br.cx), brcy = Math.round(pair.br.cy);
    var brbx = Math.round(pair.br.cx + 2 * msX), brby = Math.round(pair.br.cy);

    var tlcPi = (tlcy * width + tlcx) * 4;
    var tlbPi = (tlby * width + tlbx) * 4;
    var brcPi = (brcy * width + brcx) * 4;
    var brbPi = (brby * width + brbx) * 4;

    var threshR = ((pixels[tlcPi] + pixels[tlbPi]) / 2 + (pixels[brcPi] + pixels[brbPi]) / 2) / 2;
    var threshG = ((pixels[tlcPi + 1] + pixels[tlbPi + 1]) / 2 + (pixels[brcPi + 1] + pixels[brbPi + 1]) / 2) / 2;
    var threshB = ((pixels[tlcPi + 2] + pixels[tlbPi + 2]) / 2 + (pixels[brcPi + 2] + pixels[brbPi + 2]) / 2) / 2;
    _lastDebug.threshR = threshR; _lastDebug.threshG = threshG; _lastDebug.threshB = threshB;

    // Build pi lookup table: precompute pixel buffer offset for every data cell
    ...
    // Sample cells: lookup pi, inline threshold comparison
    // Also record per-channel 8-bit deviation from ideal (0 or 255) for quality metrics
    ...
    for (var i = 0; i < DATA_CELLS; i++) {
      ...
      var rBit = rVal < threshR, gBit = gVal < threshG, bBit = bVal < threshB;
      if (rBit) result[byi]      |= bit;
      if (gBit) result[gOff + byi] |= bit;
      if (bBit) result[bOff + byi] |= bit;
      // Channel deviation: bit=1 ideal=0, bit=0 ideal=255
      rDev[(rBit ? rVal : (255 - rVal)) >> 3]++;
      gDev[(gBit ? gVal : (255 - gVal)) >> 3]++;
      bDev[(bBit ? bVal : (255 - bVal)) >> 3]++;
    }
