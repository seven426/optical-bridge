...
const FrameProtocol = (() => {
  ...
  function decodeInnerFEC(encodedData, totalCapacity) {
    ...
    for (var b = 0; b < numBlocks; b++) {
      ...
      result.set(corrData.slice(INNER_NPAR, BLOCK_SIZE), b * INNER_K);

      // Count bit errors in data portion (K bytes) via XOR + popcount
      for (var bi = 0; bi < INNER_K; bi++) {
        totalBitErrors += POPCOUNT_TABLE[block[INNER_NPAR + bi] ^ corrData[INNER_NPAR + bi]];
      }
    }
