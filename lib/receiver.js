const Receiver = (() => {
  ...
  function init() {
    state = {
      ...
      qualityMetrics: {
        ...
        correctedBits: 0,
        channelQuality: {
          r: { samples: 0, sum: 0, sqSum: 0, hist: new Uint32Array(32) },
          g: { samples: 0, sum: 0, sqSum: 0, hist: new Uint32Array(32) },
          b: { samples: 0, sum: 0, sqSum: 0, hist: new Uint32Array(32) }
        }
      }
    };
  }
  ...
  function recordScanQuality(channelDev, bitErrors, dataBits) {
    if (!state) return;
    var qm = state.qualityMetrics;
    qm.totalScans++;
    qm.totalDataBits += dataBits;
    qm.correctedBits += bitErrors;
    if (channelDev) {
      var accum = qm.channelQuality;
      ['r', 'g', 'b'].forEach(function(ch) {
        ...
        for (var b = 0; b < 32; b++) {
          ...
          ac.hist[b] += cnt;
          ac.samples += cnt;
          var mid = b * 8 + 4;  // bucket midpoint
          ac.sum += cnt * mid;
          ac.sqSum += cnt * mid * mid;
        }
      });
    }
  }

  function recordBadScan() {
    if (!state) return;
    state.qualityMetrics.totalScans++;
    state.qualityMetrics.badScanFrames++;
  }

  function getQualityMetrics() {
    if (!state) return null;
    var qm = state.qualityMetrics;
    var cq = qm.channelQuality;
    var result = {
      totalScans: qm.totalScans,
      badScanFrames: qm.badScanFrames,
      totalDataBits: qm.totalDataBits,
      correctedBits: qm.correctedBits
    };
    ['r', 'g', 'b'].forEach(function(ch) {
      var ac = cq[ch];
      var mean = ac.samples > 0 ? ac.sum / ac.samples : 0;
      var variance = ac.samples > 0 ? (ac.sqSum / ac.samples) - mean * mean : 0;
      result[ch] = {
        samples: ac.samples,
        meanDev: mean,
  ...
  function reconstructFile() {
    ...
    var metaFrame = allDataFrames[0];
    var metadata = null;
    if (metaFrame) {
      metadata = FrameProtocol.parseMetadataPayload(metaFrame.data);
    }

    // Remaining frames are file data
    var fileChunks = [];
    for (var i = 1; i < allDataFrames.length; i++) {
    ...
    return SHA256.hash(finalBytes).then(function(hash) {
      ...
      if (metadata && metadata.sha256) {
        for (var i = 0; i < 32; i++) {
          senderSHA256 += metadata.sha256[i].toString(16).padStart(2, '0');
        }
        sha256Match = sha256hex === senderSHA256;
      }
  ...
  return { init: init, ingestFrame: ingestFrame, ingestFrameBinary: ingestFrameBinary,
           ...
           recordScanQuality: recordScanQuality, recordBadScan: recordBadScan, getQualityMetrics: getQualityMetrics };
})();
