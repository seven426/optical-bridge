const Receiver = (() => {
  ...
  function reconstructFile() {
    ...
    // Get FEC K and fileSize from first available block
  ...
  function getMissingFrames() {
    ...
    // Mark which blocks are satisfied (have >= K received frames)
    var blockReady = {};
    for (var gid = 0; gid < Object.keys(state.fecBlocks).length; gid++) {
      var block = state.fecBlocks[gid];
      blockReady[gid] = block && block.received.size >= fecK;
    }

    // Only include missing frames from blocks that still need more
    var missing = new Set();
    for (var i = 0; i < totalWithParity; i++) {
      var blk = Math.floor(i / groupSize);
      if (!blockReady[blk] && !state.receivedSet.has('0-' + i)) {
        missing.add(i);
      }
    }
    return missing;
  ...
  function getMissingBlocks() {
    if (!state || !state.fecBlocks) return new Set();
    var fecK = state.fecBlocks[0] ? state.fecBlocks[0].k : 0;
    if (!fecK) return new Set();
    var missing = new Set();
    var blockIds = Object.keys(state.fecBlocks);
    for (var b = 0; b < blockIds.length; b++) {
      var gid = parseInt(blockIds[b]);
      var block = state.fecBlocks[gid];
